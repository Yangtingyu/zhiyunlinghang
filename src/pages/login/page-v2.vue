    ...
    <div v-show="showLogin" class="login-page" :class="[theme]">
          ...
          <div class="login-form-inner">
            <div class="layer flex-center">
              ...
              <div class="form-group">
                <el-card>
                  <el-form ref="loginForm" label-position="top" :rules="rules" :model="formLogin" :show-message="false" :status-icon="false">
                    ...
                    <el-form-item v-if="isShowCode" prop="code">
                      <el-input type="text" v-model="formLogin.code" placeholder="验证码" @keyup.enter.native="submit">
                        ...
                        <template slot="append">
                          <img class="login-code" :src="loginCodeUrl" @click="changeLoginCode" style="cursor: pointer">
                        </template>
                      </el-input>
                    </el-form-item>
                    ...
                    <el-button @click="submit" type="primary" class="button-login" :loading="disabledSubmit">
                      登录
                    </el-button>
        ...
        <el-dialog title="提示" center :visible.sync="autoLoginDialogVisible" width="400px">
          ...
          <span slot="footer" class="dialog-footer">
            <el-button size="mini" icon="el-icon-delete" @click="clearRememberPasswordInfo">清除记录</el-button>
            <el-button size="mini" icon="el-icon-close" @click="cancelAutoLogin">取消登陆</el-button>
            <el-button size="mini" icon="el-icon-check" type="primary" @click="submit" :loading="disabledSubmit">立即登陆</el-button>
          </span>
        </el-dialog>
    </div>
</template>
...
import AnimationEarth from '@/pages/login/components/AnimationEarth'

export default {
  name: "login-page-v2",
  components: { AnimationEarth },
  props: {
      // 主题色
      theme: {
        type: String,
    ...
    methods: {
        ...
        submit () {
            this.$refs.loginForm.validate((valid) => {
                if (valid) {
                    ...
                    // 获取RSA信息
                    this.$axios.get('/service/sso/keyPair').then(res => {
                        if (res.code == 200) {
                            let publicKey = RSAUtils.getKeyPair(res.body.exponent, '', res.body.modulus)
                            let password = this.formLogin.password
                            let encryptedPassword = RSAUtils.encryptedString(publicKey, password)
                            this.d2adminLogin({
                                vm: this,
                                username: this.formLogin.username,
                                password: encryptedPassword,
                                mdpassword: crypto.createHash('md5').update(password).digest('hex'),
        ...
        initIsShowCode(callback) {
            this.$axios.get('/service/sso/code').then(res => {
        ...
        autoLogin(routerName) {
            let that = this
            that.$axios.get('/service/sso/keyPair').then(res => {
                if (res.code == 200) {
                    let publicKey = RSAUtils.getKeyPair(res.body.exponent, '', res.body.modulus)
                    let password = 'EverSec123!@#abc'
                    let encryptedPassword = RSAUtils.encryptedString(publicKey, password)
                    that.$axios.post('/service/sso/login', {
                        userName: 'CMISIS',
                        password: encryptedPassword,
                        ...
                        if (res.code == 200) {
                            ...
                            util.cookies.set('oauth', crypto.createHash('md5').update(password).digest('hex'))
    ...
    created() {
        ...
        if (autoLoginInfo) {
            ...
            if (tempArray.length == 2) {
                // 验证 token 的有效性
                that.$faxios.post('/service/monitoring-query/autologin/checktoken', qs.stringify({
...
// 蓝色星空主题定制
.login-page.blue-sky {
  // background-image: url(/image/cmiot-home-v3/img/blue-sky-bg4.jpg);
  background-image: url(/image/cmiot-home-v3/img/blue-sky-bg.jpg);
  // background-image: url("../login-1/image/login-bg.jpg");
  overflow: hidden;
}
</style>
